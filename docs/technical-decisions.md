...
- **Items are consumed immediately on queueing** (`queueItemAction`); a skill's MP/`usesPerCombat`/`cooldownsRemaining` are only *validated* here — they're actually committed later, at execution time (§3), so a character's resources don't look spent before their turn has come up during the round's reveal.
...
- **`damage`**: for each target, `finalDamage = max(1, round((effect.amount + mitigatedOffense(offensiveStat, target.defense)) * damageMultiplier))`, where `offensiveStat = source.attack` by default, or `source.magicPower` if the skill has `isMagic: true` **and** `source` is a `Character` or a `Summon` (a plain `Monster` always uses `attack`). The `isMagic` flag is passed down via `ResolveContext.isMagic` from `combat.ts`'s `applySkillEffects`.
  ...
  The next step after `mitigatedOffense` in the multiplication chain (`resolveSkillEffect`'s `"damage"` case) is `raceMultiplierFor(target, effect.damageType ?? "physical")`: `(1 - resist/100) * (1 + weak/100)`, where `resist`/`weak` come from the target's resolved race/subRace/trait profile (a no-op, `1`, when `target` isn't a `Monster`) — `gameplay-decisions/02-monster.md`'s "Race, subRace & traits" section. This is also where the usual `max(1, ...)` floor gets an exception: when the race multiplier is exactly `0` (100% resist), `finalDamage` is `0`, not `1`.
...
- **`applyStatusEffect`** / **`removeStatusEffect`**: adds/removes an entry `{ statusEffectId, turnsRemaining }` from `target.activeStatusEffects` (a list unique by `statusEffectId` — reapplying an already-active status always refreshes `turnsRemaining` back to `durationTurns`; for a status with `stackable: true` it also adds 1 stack up to `maxStacks`, scaling that status's own `perTurnEffects` — see `gameplay-decisions/01-class-skill.md` §1.12.6 for the mechanic and `bleeding`'s concrete numbers). A stack gain logs `resolver.statusStack` instead of the plain `resolver.statusRefresh`, and the current stack count (once >1) is shown next to the effect in the party/monster status listings.
...
- For items, consumption happens in the command phase, at the moment the `QueuedAction` is created (§2). For skills, MP deduction happens at execution time, inside `runCharacterTurn` — not when the resolver runs, and not at queue time either (only validated then). The resolver only handles the effect itself, not the cost of activating it.
- `usesPerCombat` is also checked (command phase) & decremented (execution phase, alongside MP) the same way.
...
### 4.4 Dual-side skills — different effects for ally/enemy (`SkillEffect.appliesToRelation` + `SkillEffect.target` + 2 new `SkillTarget` values)
Used for Acolyte's Purify (choose one of two sides), Divine Descent (both sides at once), and Plague Doctor's Total Plague (both sides at once); also used more broadly by any skill that needs one effect to land somewhere other than the skill's own `target`.
- 2 `SkillTarget` values cover the "both/either side" shape: `"singleAllyOrEnemy"` (Purify — the player picks a single target, which can be an ally or an enemy) and `"allAlliesAndEnemies"` (Divine Descent, Total Plague — automatically targets everyone on both sides).
- `SkillEffect.appliesToRelation?: "ally" | "enemy"` narrows one effect to only the side of the population already resolved by the skill's own `target` — e.g. Purify's `removeStatusEffect` effect has `appliesToRelation: "ally"` and its `damage` effect has `appliesToRelation: "enemy"`, both still resolved against whatever `target` picked out. This is a *filter within* an already-resolved population, not a separate resolution — the only shape that correctly expresses Purify, whose `singleAllyOrEnemy` target is exactly 1 player-picked actor whose side decides which effect fires.
- `SkillEffect.target?: SkillTarget` instead *overrides* the skill's own `target` for one effect, resolving it against a wholly separate population — e.g. a `summon` effect always lands on the caster (`target: "self"`) even when the skill's own `target` is `allEnemies`. Only self/all-* targets are supported (a `single*` override is skipped — there's no UI to pick a 2nd target mid-cast).
- `applySkillEffects` (`src/engine/combat.ts`): `isOverrideEffect(e)` is `e.kind === "summon" || (e.target !== undefined && e.target !== skill.target)`. Override effects are resolved once each, against their own `effect.target ?? "self"` (for `summon`) or `effect.target` population, before the main per-target loop runs. Every other effect goes through the normal per-target loop as before (1 accuracy roll per target, shared by every effect on that target); an effect with `appliesToRelation` set is skipped there for any target on the wrong side.
...
- The AoE accuracy roll (`gameplay-decisions/04-fear-combat.md` §4.1) applies to the `enemy`-facing effects of these skills; `ally`-facing effects don't roll.
...
- `applySkillEffects`: if `skill.isUltimate` and `isCharacter(source)`, `scaleEffectForUltimate` multiplies every `damage`/`heal` effect's `effect.amount` **and** `effect.offenseMultiplierPercent` by a coefficient looked up from `ultimateEffectivenessMultiplier(fear)` (`src/engine/combat.ts`, keyed off the caster's fear tier) before passing it to `resolveSkillEffect` — both terms scale together, since scaling `amount` alone left the stat-scaled (usually larger) portion of the effect untouched by fear. 2 separate fear-based weakening mechanisms run in parallel for 2 different skill groups: ultimates use their own multiplier function here, regular skills use `getFearDamagePenalty` (`gameplay-decisions/04-fear-combat.md` §4).
...
- `queueAction`: blocks if `(actor.cooldownsRemaining[skillId] ?? 0) > 0`. `cooldownsRemaining[skillId]` is set at execution time (`runCharacterTurn`, alongside MP deduction — §3), to `Math.max(0, skill.cooldownTurns - totalCooldownReduction(actor))` — `totalCooldownReduction` (`src/engine/artifacts.ts`) sums any cooldown-reduction granted by the actor's equipped artifacts, so the stored cooldown can be lower than the skill's own `cooldownTurns`.
...
- Convention for assigning `cooldownTurns` (`gameplay-decisions/01-class-skill.md` §1.5): for skills with `isBuff: true` (Shield Guard, Rally, Poison Coat) → `cooldownTurns = the skill's own applyStatusEffect effect's durationTurns + 1`; damage/utility skills are assigned by hand; ultimates share a fixed value across every class (see `data/classes.json`).
...
- **Exception: a skill with a `summon` effect never gets this boost, even if it's also `isBuff: true`** (Summon Goblin/Spirit/Golem, Totem Recall, Ninja's Shadow Clone) — `turnOrderSortKey` checks `isSummonSkill` (any effect with `kind: "summon"`) and skips the bonus when true, so these resolve in plain speed order alongside every other attack/utility skill. `isBuff: true` is kept on them only for the unrelated cooldown convention (§4.6) and to keep them out of Blood Fury's damage bonus (§1.7-adjacent, `applySkillEffects`'s `vikingBonus` check) — not for turn order.
- Has no effect on monsters (monsters don't pre-queue).
- If several characters use an `isBuff: true` skill (with no summon effect) in the same round, all get the same boost — ties are still broken by the old rule (equal `speed` → characters before monsters, then original order within `combatants`).

#### 4.7.1 A summon faster than its owner acts the instant it's cast — `SummonArchetype.speed`

A summon spawned mid-round is never in that round's `turnQueue` (`buildTurnQueue` only sees combatants that already existed when the round started), so without this it would sit idle until next round regardless of how fast it is.

- `SummonArchetype` gained a required `speed: number` field (`data/summons.json`) — a flat, fixed trait of the creature itself, **not** derived from the owner the way every other minion stat is (§1.12.4, `gameplay-decisions/01-class-skill.md`). `addSummon` (`src/engine/combat.ts`) sets `Summon.speed` straight from `archetype.speed` at spawn time, and it stays that value for the summon's whole lifetime — it does **not** track the owner's `speed` afterward either (a later buff/debuff to the owner's speed doesn't touch an already-summoned minion).
- Right after `applySkillEffects` resolves the casting skill, `runCharacterTurn` checks the returned spawned summon (if any): `spawnedSummon.speed > actor.speed` → `runSummonTurn` is called immediately, so the summon gets a real extra action that round (it counts toward its `maxActions`, same as any other turn). Equal or lower speed → no immediate action; it just waits for its own place in next round's `turnQueue`, exactly like before this mechanic existed.
- Shipped values (tunable, no code change needed to retune): `stone-golem: 6` (slower than every class — a cast-and-it-waits tank, matching its "heavy" theme), `goblin-thrower: 12`, `healer-spirit: 14`, `ninja-clone: 16`, `hellfire-imp: 18` (all faster than most classes' base `speed`, 8-17). `recall-totem: 1` is a placeholder — irrelevant in practice since `archetype.passive` skips it ever taking a turn at all.

### 4.8 Combat-stat buff/debuff floor (`SkillEffect.minPercent`) and a scaled on-hit AoE splash (`onHitAoeDamage.offenseMultiplierPercent`)
A flat `modifyCombatStat` amount (Guard's `+6 defense`, Weakened's `-6 defense`, ...) is worth a lot relative to a level-1 character's stats and next to nothing relative to a level-100 one — the same problem `maxHpPercent` (§1.7.1, `gameplay-decisions/01-class-skill.md`) solved for DoTs, applied to stat buffs/debuffs instead of HP ticks.
- New optional field on `SkillEffect`: `minPercent?: number`, set on a `modifyCombatStat` entry inside a status's `perTurnEffects`. The delta actually applied is whichever of `amount` or `round(actorStat * minPercent / 100)` has the larger magnitude, sign preserved — `actorStat` is read *before* this delta lands, so the buff/debuff can't include itself in its own floor. A negative `minPercent` floors a debuff the same way a positive one floors a buff (e.g. Weakened: `amount: -6, minPercent: -10` — at high defense the applied delta is the more-negative percent value, not the flat `-6`). A tie keeps the flat `amount`. `aggro` is deliberately never given a `minPercent` in the shipped data — unlike `attack`/`defense`/`magicPower`, `aggro` doesn't grow with level (`statsForLevel`, `src/engine/party.ts`), so a flat aggro bonus never becomes relatively negligible the way an attack/defense one does.
- New field on `ActiveStatusEffect`: `appliedAmounts?: Partial<Record<CombatStat, number>>` — the exact delta(s) `applyStatusEffectToActor` (`src/engine/resolver.ts`) computed and applied for each `modifyCombatStat` entry, stored once at application. `expireStatusEffect` undoes `appliedAmounts[stat]`, not a fresh recompute of `amount`/`minPercent` — the actor's stat can move between application and expiry (a 2nd, unrelated buff/debuff on the same stat), and recomputing at that point would floor against the wrong baseline and undo the wrong amount.
- The direct `case "modifyCombatStat"` in `resolveSkillEffect` (reached only via a status's `perTurnEffects` today, since no skill/item currently uses `modifyCombatStat` as one of its own `effects`) goes through the same `computeCombatStatDelta` helper, so the 2 code paths can't drift.
- New optional field on `StatusEffectDefinition.onHitAoeDamage`: `offenseMultiplierPercent?: number` — same meaning as the existing `SkillEffect.offenseMultiplierPercent` (§4 damage formula), scaling the caster's `attack`/`magicPower` contribution before mitigation. `applyOnHitAoeDamage` (`src/engine/combat.ts`) builds its `damage` effect with this field set — `storm-empowered`/`-ii`/`-iii` set it to `80`/`90`/`100`.

### 4.9 Two description fields — full detail vs. compact list (`SkillDefinition.description` / `.shortDescription`)
- `description: string` — the full, unabridged writeup. Shown only on the `skillDetail` confirm screen (`src/ui/screens/combat.ts`) reached after picking a skill from the `pickSkill` list. No length limit — this is where a long writeup (Divine Descent, Total Plague) belongs.
- `shortDescription?: string` — a terser rewrite of the same skill for the compact `pickSkill` list line (`  [n] Name (MP x) — {{shortDescription}}`), which also carries the skill's name and MP cost ahead of it on the same line. **Limit: 50 characters.** Sized against the Dungeon panel's real content width at the project's standard 130-column test terminal (`tui-testing` skill) — 62 usable characters per line total, leaving enough room after the longest name/MP-cost prefix (~33 chars) + the ` — ` separator that only a skill with both a long name and a long `shortDescription` wraps to a 2nd line — a graceful word-wrap, never a truncation ellipsis.
- `pickSkill`'s render falls back to `description` when `shortDescription` is unset (`sk.shortDescription ?? sk.description`) — every class skill in `data/classes.json` sets both today, but the fallback keeps any future skill from rendering `undefined`.
- `data/monster-skills.json` reuses the same `SkillDefinition` type but never sets `shortDescription` — monster skills are AI-driven and never render through `pickSkill`, so the field is meaningless there.
...
- **One skill list, tier eligibility carried by the weights.** `MonsterArchetype.skillIds` holds every skill the archetype can roll at any tier, and `actionWeights` maps, per tier, `"basicAttack"` plus each usable skill id to its weight. A skill is usable at a tier exactly when that tier's weights name it, so there is no separate allow-flag that could disagree with a weight. `executeSkillId` stays a field of its own because it never goes through the weighted roll at all. `assertMonsterDataConsistent` (`src/data/monsters.ts`) runs when the catalog loads and rejects a weight key that names no declared skill; before that, a typo silently left the monster on basic attacks forever.
- **`MonsterArchetype.roles: MonsterTier[]`** is the single source of truth for where an archetype can spawn (trash `["normal"]`, triple `["normal","elite","boss"]`, guard-only `["elite","boss"]`, final boss `["boss"]` + `finalBoss: true`). Pools derive from it (`COMBAT_ROOM_ARCHETYPES`/`GUARD_ROOM_ARCHETYPES`/`FINAL_BOSS_ARCHETYPE` in `src/data/monsters.ts`); exactly 1 final boss is enforced at load.
- **`runMonsterTurn`** branches by `actor.tier` before reaching the targeting logic. If it's not a charge/release turn: decrement `executeCooldownTurns`, then the actual action is picked in one shot by `pickMonsterAction` — a single weighted random choice over whichever candidates are currently eligible (filtered by tier and by which archetype fields are set), not a sequence of independent per-action rolls with a fallback. The weights themselves are **not** in the engine — they're per-archetype data, field `actionWeights` in `data/monsters.json`, and Debuff uses a different status depending on archetype (`data/monster-skills.json`). No skill or archetype names are hardcoded in the engine.
- **Strike/Cleave/Debuff and a regular monster's own skill share one branch** (`src/engine/monsterAI.ts`). The action-weight key names a skill and nothing more: `skillIdForWeightKey` resolves it, and from there both targeting (`resolveMonsterSkillTargets`) and the log line (`monsterSkillLogLine`) derive from the skill's own `target` field — `allEnemies` hits every living character, `self` hits the monster, anything else goes through `pickMonsterTarget`. Debuff doesn't call `pickAggroWeighted` directly; it follows the archetype's own `aiPattern` like every other single-target skill. Vampire Lord and Lich are `opportunistic`, so their Debuff lands on the party's quietest character rather than its loudest.
- **Log lines are derived, not assigned per role**: `combat.monsterSkillAoe` for an `allEnemies` skill, `combat.useSkillPlain` for a `self` skill, `combat.monsterSkillOnTarget` otherwise.
