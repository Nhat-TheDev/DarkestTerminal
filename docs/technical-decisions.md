...
- New optional field on `StatusEffectDefinition.onHitAoeDamage`: `offenseMultiplierPercent?: number` — same meaning as the existing `SkillEffect.offenseMultiplierPercent` (§4 damage formula), scaling the caster's `attack`/`magicPower` contribution before mitigation. `applyOnHitAoeDamage` (`src/engine/combat.ts`) previously built its `damage` effect without this field, so an on-hit AoE splash (Viking's `storm-empowered`) always used the caster's full, unscaled stat on every hit regardless of rank; `storm-empowered`/`-ii`/`-iii` now set it to `80`/`90`/`100`.
...
- `shortDescription?: string` — a terser rewrite of the same skill for the compact `pickSkill` list line (`  [n] Name (MP x) — {{shortDescription}}`), which also carries the skill's name and MP cost ahead of it on the same line. **Limit: 50 characters.** Sized against the Dungeon panel's real content width at the project's standard 130-column test terminal (`tui-testing` skill) — 62 usable characters per line total, leaving enough room after the longest name/MP-cost prefix (~33 chars) + the ` — ` separator that only a skill with both a long name and a long `shortDescription` wraps to a 2nd line — a graceful word-wrap, never a truncation ellipsis (which `pickSkill` no longer does at all: it used to hard-truncate `description` to 34 chars with `truncateText`, before `shortDescription` existed to write around the limit instead of cutting it at render time).
...
- **Strike/Cleave/Debuff and a regular monster's own skill share one branch** (`src/engine/monsterAI.ts`). The action-weight key names a skill and nothing more: `skillIdForWeightKey` resolves it, and from there both targeting (`resolveMonsterSkillTargets`) and the log line (`monsterSkillLogLine`) derive from the skill's own `target` field. They used to be 4 separate cases, which was 4 copies of the same behavior — `allEnemies` hits every living character, `self` hits the monster, anything else goes through `pickMonsterTarget`. One consequence worth knowing: Debuff no longer calls `pickAggroWeighted` directly, so it follows the archetype's own `aiPattern` like every other single-target skill. Vampire Lord and Lich are `opportunistic`, so their Debuff lands on the party's quietest character rather than its loudest — deliberate, and the reason the note is here.
- **Log lines are derived, not assigned per role**: `combat.monsterSkillAoe` for an `allEnemies` skill, `combat.useSkillPlain` for a `self` skill, `combat.monsterSkillOnTarget` otherwise. (The first two of those keys were named `combat.eliteCleave`/`combat.eliteStrike` while they were Elite-only.) This is why a plain monster's skill and a boss's Debuff now name their target, which neither did before.
