...
- **`modifyStat`**: `target.survival[effect.stat] += effect.amount` (`fear`/`hunger`/`thirst`, `Character` only), then clamped to the survival-stat range (`gameplay-decisions/03-survival-stats.md` §3).
...
- `buildTurnQueue`: when computing the sort key for a character combatant, checks `combat.queuedActions` to see if that actor has a `QueuedAction` pointing at a skill with `isBuff: true` → if so, uses a boosted sort key instead of `actor.speed` (the bonus amount is a named constant in `src/engine/combat.ts`). This does **not** overwrite the actor's real `speed` — it's only a temporary value used for sorting, discarded as soon as `turnQueue` is built.
...
- **`runMonsterTurn`** branches by `actor.tier` before reaching the old targeting logic (`pickMonsterTarget`, shared by both regular monsters and the Elite/Boss Strike skill). If it's not a charge/release turn: decrement `executeCooldownTurns`, then roll for Debuff (a different status depending on archetype — see `data/monster-skills.json`), then for both Elite and Boss tiers roll for Cleave, finally falling back to Strike — generic based on `archetype.eliteSkillIds`/`bossSkillIds`, with no skill/archetype names hardcoded in `combat.ts`. The proc chances used for these rolls are named constants in `src/engine/combat.ts`.
