...
## 5. Elite/Boss skill kit (docs/gameplay-decisions.md §6.12)
...
- **Data mới**: `data/monster-skills.json` (song song `data/classes.json`, nhưng phẳng — không nhóm theo class) + `getMonsterSkill(id)` ở `src/data/monsters.ts`. 4 entry (`skeleton-elite-strike`, `skeleton-elite-cleave`, `skeleton-boss-execute`, `skeleton-boss-crush`) tái dùng nguyên `SkillDefinition` type — `mpCost`/`slot`/`unlockLevel` bị bỏ qua khi resolve (chỉ có ý nghĩa cho player skill qua `queueAction`), giữ giá trị placeholder (`0`/`0`/`1`) để không cần thêm type mới.
- **`MonsterArchetype.eliteSkillIds`/`bossSkillIds`** (optional, `types.ts`): archetype nào không set thì tier elite/boss của nó vẫn rơi về đòn đánh thường cũ (`amount 0`) — hiện chỉ `skeleton-guard` set, vì đây là archetype duy nhất từng được spawn ở tier elite/boss (`BOSS_ARCHETYPE_ID` cố định ở `floor.ts`).
- **`runMonsterTurn` (combat.ts)** rẽ nhánh theo `actor.tier` trước khi tới logic targeting cũ (`pickMonsterTarget`, tách ra từ nhánh `else` gốc để dùng chung cho cả quái thường lẫn Chém Hạ Gục của elite/boss). Nếu không phải charge/release turn: decrement `executeCooldownTurns`, rồi roll `ctx.rng.chance(0.3)` cho Nghiền Nát, rồi tới elite/boss chung roll `ctx.rng.chance(0.3)` cho Chém Quét, cuối cùng fallback Chém Hạ Gục.
- **Đòn Kết Liễu — tích lực 2 lượt thay vì trigger theo %HP (cập nhật lần 2, §6.12)**: state mới trên `Monster` (`types.ts`) — `executeCooldownTurns` (khởi tạo `EXECUTE_COOLDOWN_TURNS = 3` ở `spawnMonster`, chỉ cho tier boss), `isChargingExecute`, `executeTargetId`. Mỗi lượt boss (đầu `runMonsterTurn`, trước cả nhánh Nghiền Nát/Chém Quét):
  1. `isChargingExecute === true` → đọc `executeTargetId` (fallback `pickAggroWeighted` nếu mục tiêu đã chết giữa 2 lượt), gọi `applySkillEffects` với `skeleton-boss-execute` (`amount 71`), reset `isChargingExecute = false`, `executeTargetId = undefined`, `executeCooldownTurns = EXECUTE_COOLDOWN_TURNS`, rồi `return` — không rơi xuống nhánh Nghiền Nát/Chém Quét/Chém Hạ Gục cùng lượt.
  ...
  3. Ngược lại → decrement `executeCooldownTurns`, tiếp tục rẽ nhánh Nghiền Nát/Chém Quét/Chém Hạ Gục như cũ.
  - Vì mục tiêu được chọn 1 lần lúc tích lực (bước 2) và đọc lại nguyên vẹn lúc tung đòn (bước 1), thay đổi `aggro` **sau** khi đã khoá không ảnh hưởng gì — chỉ có tác dụng nếu dùng **trước** lượt tích lực trong cùng round (nhờ rule +20 speed cho buff, mục 1, khiến Che Chắn kịp resolve trước lượt boss nếu dùng cùng round).
  - `hasStunningStatus` check ở đầu `runMonsterTurn` vẫn chạy trước toàn bộ logic trên — nếu boss bị choáng đúng lượt lẽ ra tung Kết Liễu, `isChargingExecute` **giữ nguyên `true`** (turn bị bỏ qua hoàn toàn, không đọc tới nhánh boss), nên đòn vẫn tung ra ở lượt kế tiếp không bị choáng — stun trì hoãn chứ không hủy đòn đã tích lực.
