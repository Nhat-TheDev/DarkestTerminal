...
fixed regardless of which branch is chosen:
- Stage 0 (start) is exactly 1 room, tag `""` (normal room).
- The final stage is exactly 1 room, tag `boss`.
- Path length (total rooms, including start + boss): **7–12 rooms** (`MIN_PATH_ROOMS`/`MAX_PATH_ROOMS`), randomized on each generation.
- **Branch points** (branch = a stage with >1 room) can only be placed from the 3rd room onward (`MIN_BRANCH_START_STAGE = 2`, 0-indexed).
- At most **3 branch points** (`MAX_BRANCHES`); 2 consecutive branch points must be at least **3 rooms apart** (`MIN_BRANCH_SPACING`) — so a short path (7 rooms) can only fit ~2 branch points at most.
...
- Because branch points are spaced ≥3 rooms apart, no 2 event rooms are ever adjacent on the same path.
- At most **4 event rooms** per path (`MAX_EVENT_ROOMS_PER_PATH`).
- **1–2 rest rooms** per path (`MIN_REST_ROOMS_PER_PATH`/`MAX_REST_ROOMS_PER_PATH`) — chosen randomly among stages that aren't start/boss/branch points.
...
randomly places 1-3 monsters per combat room (11 normal combat archetypes,
`gameplay-decisions/02-monster.md` §2), 1 elite or boss monster (5 guard-room
archetypes, randomly picking 1 —
...
over 200 seeds, verifying all the rules above plus reachability/no-dead-end (BFS),
...
- The player picks an action (skill or item) + target for **all 4 living characters** up front, without seeing what the monsters will do this round. Monsters do **not** act in this phase.
...
- Once all 4 `QueuedAction`s are in → `phase` moves to `"resolution"`.

### Phase 2 — Execution (`phase: "resolution"`)
- Build a `turnQueue` = every living combatant (4 characters + all monsters), taking a `speed` snapshot at exactly this moment (not recalculated mid-round even if `speed` buffs/debuffs occur during execution). Sort descending by `speed`; ties → characters before monsters, then original order within `combatants`.
...
- **`damage`**: for each target, `finalDamage = max(1, round((effect.amount + mitigatedOffense(offensiveStat, target.defense)) * damageMultiplier))`, where `offensiveStat = source.attack` by default, or `source.magicPower` if the skill has `isMagic: true` **and** `source` is a `Character` (monsters always use `attack`). The `isMagic` flag is passed down via `ResolveContext.isMagic` from `combat.ts`'s `applySkillEffects`.
  `mitigatedOffense(off, def) = off − off·(def/(60+def)) − def/30` (constants `DEFENSE_MITIGATION_X=60`/`DEFENSE_MITIGATION_Y=30`, `src/engine/resolver.ts`).
  If `source` is a character and currently at the fear tier "Anxious"/"Panicked" (`gameplay-decisions/04-fear-combat.md` §4), an additional accuracy roll and matching damage multiplier are applied before HP is deducted; the "Breakdown" tier additionally has a chance to skip the turn, rolled at the action-selection step (command phase), before the resolver is ever called. **Exception when `source === target`** (a status effect's own periodic tick on the actor carrying it, e.g. the "Poisoned" DoT — `tickStatusEffects` calls `resolveSkillEffect` again with `source`/`target` being the same actor): this isn't "an attack", so attack (or magicPower)/defense are **not** added/subtracted and the fear multiplier isn't applied — the damage is a fixed `effect.amount`.
...
- **`modifyStat`**: `target.survival[effect.stat] += effect.amount` (`fear`/`hunger`/`thirst`, `Character` only), then clamped to `[0, 100]`.
...
- A `SkillDefinition`/`ItemDefinition` can have multiple `effects`; the resolver applies them **sequentially in array order**, with each later effect seeing state already changed by earlier ones (e.g. Rogue's Flurry Assault = 3 consecutive `damage` effects, where effect 2 is computed on HP already reduced by effect 1).
...
## 4. Skill mechanics for the 6-skill-per-class kit
...
- An optional field on `SkillEffect`: `chance?: number` (0-1). Not having this field means it always applies.
...
- 2 new `SkillTarget` values: `"singleAllyOrEnemy"` (Purify — the player picks a single target, which can be an ally or an enemy) and `"allAlliesAndEnemies"` (Divine Descent — automatically targets everyone on both sides).
...
- `applySkillEffects`: if `skill.isUltimate` and `isCharacter(source)`, multiplies every `damage`/`heal` effect's `effect.amount` by a coefficient from the fear-ultimate table (`gameplay-decisions/04-fear-combat.md` §4.1) before passing it to `resolveSkillEffect` — 2 separate fear-based weakening mechanisms run in parallel for 2 different skill groups: ultimates use their own coefficient here, regular skills use a flat 15%.
...
- Convention for assigning `cooldownTurns` (`gameplay-decisions/01-class-skill.md` §1.5): for skills with `isBuff: true` (Shield Guard, Rally, Poison Coat) → `cooldownTurns = the main status's durationTurns + 1`; damage/utility skills are assigned by hand; ultimates are fixed at `5`.

### 4.7 Buffs always act first in the round — `SkillDefinition.isBuff` + a temporary speed bonus for turn-order calculation
Used for Shield Guard, Rally (Vanguard), Poison Coat (Rogue) — the only 3 skills with `isBuff: true`.

- An optional field on `SkillDefinition`: `isBuff?: boolean`.
- `buildTurnQueue`: when computing the sort key for a character combatant, checks `combat.queuedActions` to see if that actor has a `QueuedAction` pointing at a skill with `isBuff: true` → if so, uses `actor.speed + 20` as the sort key instead of `actor.speed`. This does **not** overwrite the actor's real `speed` — it's only a temporary value used for sorting, discarded as soon as `turnQueue` is built.
- Has no effect on monsters (monsters don't pre-queue).
- If 2+ characters use an `isBuff: true` skill in the same round, both get +20 — ties are still broken by the old rule (equal `speed` → characters before monsters, then original order within `combatants`).
...
- **Data**: `data/monster-skills.json` (parallel to `data/classes.json`, but flat — not grouped by class) + `getMonsterSkill(id)` in `src/data/monsters.ts`. Reuses the `SkillDefinition` type as-is — `mpCost`/`slot`/`unlockLevel` are ignored when resolving, keeping placeholder values. 20 entries (4 skills × 5 guard-room archetypes — Skeleton Guard, Giant Spider, Dragon, Zombie Knight, Dark Knight).
- **`MonsterArchetype.eliteSkillIds`/`bossSkillIds`** (optional): for any archetype that doesn't set these, its elite/boss tier falls back to a basic attack (`amount 0`). `MonsterArchetype.guardOnly?: boolean` marks an archetype as only eligible to be picked as a guard-room monster, never appearing in normal combat rooms (`src/data/floor.ts`, `GUARD_ROOM_ARCHETYPES`/`COMBAT_ROOM_ARCHETYPES`) — a guard room does `rng.pick(GUARD_ROOM_ARCHETYPES)`, randomly picking 1 of the 5 archetypes each time it's built.
- **`runMonsterTurn`** branches by `actor.tier` before reaching the old targeting logic (`pickMonsterTarget`, shared by both regular monsters and the Elite/Boss Strike skill). If it's not a charge/release turn: decrement `executeCooldownTurns`, then roll `ctx.rng.chance(0.3)` for Debuff (a different status depending on archetype — `weakened`/`poisoned`/`burning`/`stunned`), then for both Elite and Boss tiers roll `ctx.rng.chance(0.3)` for Cleave, finally falling back to Strike — generic based on `archetype.eliteSkillIds`/`bossSkillIds`, with no skill/archetype names hardcoded in `combat.ts`.
- **Execute — a 2-turn charge instead of an %HP trigger**: state on `Monster` — `executeCooldownTurns` (initialized to `EXECUTE_COOLDOWN_TURNS = 3` in `spawnMonster`, boss tier only), `isChargingExecute`, `executeTargetId`. Every boss turn (at the very start of `runMonsterTurn`, before even the Debuff/Cleave branch):
  1. If `isChargingExecute === true` → read `executeTargetId` (falling back to `pickAggroWeighted` if the target died in between the two turns), call `applySkillEffects` with the correct archetype's execute skill (`amount 71`), reset `isChargingExecute = false`, `executeTargetId = undefined`, `executeCooldownTurns = EXECUTE_COOLDOWN_TURNS`, then `return`.
  ...
  - The target is chosen once during the charging step (step 2) and read back unchanged when the attack lands (step 1) — changing `aggro` after it's locked in has no effect, unless it's done before the charging turn in the same round (thanks to the +20 speed rule for buffs, §4.7).
