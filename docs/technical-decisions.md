...
- **`MonsterArchetype.guardOnly?: boolean`** marks an archetype as only eligible to be picked as a guard-room monster, never appearing in normal combat rooms (`src/data/floor.ts`, `GUARD_ROOM_ARCHETYPES`/`COMBAT_ROOM_ARCHETYPES`) — a guard room does `rng.pick(GUARD_ROOM_ARCHETYPES)`, randomly picking one of the guard-room archetypes each time it's built. `scriptedOnly?: boolean` goes further and removes the archetype from every roll including that one, for a monster spawned only by name from a scripted call site.
