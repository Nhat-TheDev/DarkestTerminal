...
- **Ultimate skills** (the skill in slot 5, `isUltimate: true`): **entirely bypass** both the accuracy roll and the damage-reduction rule from the table above — an ultimate always succeeds. Instead, its **effectiveness** — both the flat `amount` and the stat-scaled `offenseMultiplierPercent` of every `damage`/`heal` effect in the skill — is multiplied by a coefficient looked up from `ultimateEffectivenessMultiplier(fear)` (`src/engine/combat.ts`, `scaleEffectForUltimate`), keyed off the caster's fear tier, **before** being plugged into the normal damage/healing formula. Both terms scale together so the whole pre-mitigation power budget shrinks with fear, not just the (usually minor) flat portion — the multiplier decreases tier over tier, read the function for the current values.
