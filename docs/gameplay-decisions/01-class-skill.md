...
**Mage's `mage-bludgeon` was renamed to Arcane Bolt** as part of this: it previously had no
`isMagic` flag at all (scaled off `attack`, description "Strikes with a staff"), which would have
been internally contradictory once tagged `damageType: "magic"`. It's now `isMagic: true`
(scales off `magicPower`) with an updated name/description to match.
...
| 4 | `summoner-totem-recall` | Totem Recall | self | Plants a passive totem (`recall-totem` — never acts, only leaves combat by actually dying; 1.13's `SummonArchetype.passive`) and applies an `attack` buff to every ally except summons, lasting **until the totem dies**, not a fixed duration — see 1.13 for the mechanism and the Bound Legion passive that replaced Mastery Summoner's old minion-HP/attack buff. | ✅ | — |
...
- Default cap (Bound Legion passive not yet at rank 2/3, see 1.13): **1 minion** active at a time — casting a new summon skill dismisses whichever minion is currently active. Rank 2 → up to **2** minions of **different types** simultaneously; rank 3 → up to **3**. Purely level-gated (5/20/35), independent of anything being cast — unlike the old Mastery Summoner skill this replaced, there is no separate "cast the buff to raise the cap" step.
...
| Rogue | Venomcraft | +15/20/30% of the hit's damage + 5/10/20 flat, as bonus `poison`-type damage | Triggers only when the target already carries a `poisoned`-family status; the bonus is a true flat amount (`ignoreDefensePercent: 100` + `offenseMultiplierPercent: 0` zero out the usual attack-vs-defense term) | `onDamageDealt` hook, `combatHooks.ts` |
| Mage | Arcane Shred | -5/-8/-12 flat or -5/-7/-10% of current defense (whichever larger), stacking up to 3× | Every hit stacks the `mage-shred` status on the target; needed a real fix to `applyStatusEffectToActor`'s existing-status branch (it only bumped `stacks`/`turnsRemaining` before, never re-applied a fresh delta per stack) | `onDamageDealt` hook, `combatHooks.ts`; stacking fix in `src/engine/resolver.ts` |
...
| Viking | Blood Fury | Below 40/45/50% HP: outgoing damage +20/30/40%; costs a flat 3% maxHp self-damage per landed attack (fixed across all ranks, not scaled) | Composes into the existing `stealthBreak.damageBonusPercent` parameter `resolveOneDamageEffect` already threads through, rather than a parallel bonus mechanism; never triggers on an `isBuff` skill | `applySkillEffects`, `src/engine/combat.ts` |
...
**The `minPercent`-override mechanic, reused for Mage and Totem Recall**: a status normally applies its own fixed, JSON-declared `modifyCombatStat` magnitude — but Mage's passive and Totem Recall (1.11, slot 4) each need **1 status id** to express **3 different magnitudes** (1 per passive/skill rank), and adding 3 near-duplicate status ids per case was rejected as needless duplication (the same reasoning that keeps `guard`/`rally` as 1 status each with rank variants only where the shape genuinely needs a distinct `tickCategory`, section 1.8). Instead, an `applyStatusEffect` `SkillEffect` may now carry `amount`/`minPercent` directly (fields that already existed on `SkillEffect` for other kinds, reused here rather than adding new ones) to **override** the target status's own declared magnitude for that one application — `mage-shred`'s and `totem-recall-buff`'s own JSON entries carry placeholder magnitudes that are never actually used unmodified; the real numbers live on each rank's `applyStatusEffect` effect in `data/classes.json` instead.
