...
When the drop roll succeeds, the specific item is chosen from a **combined pool** = the shared common-item catalog (below) + the item(s) specific to the exact `archetypeId` just killed (the "monster-specific items" table below):

- The split between the monster-specific pool and the shared common pool is a fixed even split, hardcoded in `rollItemDrop` (`src/data/items.ts`) rather than exposed as a JSON constant — **1 monster can belong to multiple groups at once** (e.g. Zombie Knight belongs to both the Zombie group and the Knight/Warrior group) → when a monster has several applicable specific items, its share of the pool is split evenly across them. Every archetype in `data/monsters.json` already has at least 1 specific item (no fallback case).
- Item weight additionally grows with floor depth via `items.itemWeightDepthGrowth` (`ITEM_WEIGHT_DEPTH_GROWTH`, `src/data/items.ts`).
