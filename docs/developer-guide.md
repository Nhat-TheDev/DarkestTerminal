...
- **Character selection at game start + save/load** — a party of 4 characters manually chosen from the 6 classes on the character select screen; 5 fixed save slots — New Game picks a slot (an occupied one asks before it is overwritten), every save (`[q]`, `[s]`, the autosave on reaching a new floor) writes to the run's slot, Continue lists the occupied slots, and any ending other than `victory` clears the slot (permadeath). Playtime is stored per save in `meta.playTime` — `src/ui/characterSelect.ts`, `src/ui/saveSelect.ts`, `src/engine/save.ts`
