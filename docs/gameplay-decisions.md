...
**⚠️ Cần tính lại (2026-08-16)**: toàn bộ bảng số bên dưới tính bằng skill/số liệu của bộ kit **5 skill/class cũ** (đặc biệt mốc "Chém Khiên `amount 10`" của Cận Vệ không còn tồn tại đúng nghĩa cũ — nay tách thành đòn đánh thường `amount 0` + skill riêng "Ném Khiên" `amount 10` có tốn MP, xem mục 1). Câu "Tu Sĩ không có skill gây damage nên không tính TTK" cũng **không còn đúng** — Tu Sĩ nay có đòn đánh thường (Đấm), Thanh Tẩy (nhắm địch) và Thần Giáng đều gây sát thương. Giữ nguyên bảng cũ bên dưới làm tham khảo lịch sử; số liệu chính thức cho bộ kit 6 skill/class + cooldown/proc mới sẽ tính lại ở bước implement, sau khi số MP/damage/cooldown ở mục 1 được chốt qua playtest.
...
Đọc kết quả: **Sát Thủ và Pháp Sư** (2 class attack cao) giữ TTK boss ổn định quanh 5-8 round suốt 1-100 — đúng vai trò "carry sát thương" của cả game, không bao giờ chạm ngưỡng bất tử. **Cận Vệ** (tank, attack weight thấp nhất trong các class có đánh) chậm dần rõ rệt (2→20 round) — **đây là chủ đích**, không phải lỗi cân bằng: Cận Vệ được thiết kế để gánh chịu đòn và giữ chân quái (`baseAggro` cao nhất, skill Khiêu Khích/Trấn Thủ), không phải để solo boss; trong party thật, Cận Vệ đánh xen kẽ Sát Thủ/Pháp Sư nên TTK thực tế của cả đội bám theo cột nhanh nhất, không theo cột Cận Vệ. **Tu Sĩ** ở bản kit cũ không có skill gây damage (thuần hỗ trợ, xem 6.8) nên không tính TTK — vai trò của Tu Sĩ là heal/hạ fear, không phải diệt quái. **⚠️ Không còn đúng ở bộ kit mới (mục 1, cập nhật 2026-08-16)**: Tu Sĩ nay có đòn đánh thường (Đấm) + Thanh Tẩy/Thần Giáng khi nhắm địch — TTK Tu Sĩ cần tính lại, dù vai trò chính vẫn là support chứ không phải carry sát thương.
...
| **Level tầng ngục** (`Floor.depth`) | Hạ boss của tầng hiện tại | Tăng `depth` thêm 1 khi phòng boss được dọn sạch, sinh tầng mới | **Không giới hạn** — xem 6.10 |
...
**Công thức EXP quái (cộng vào `partyExp` khi giết)**: tái dùng đúng cơ chế `growthBonus` đã có ở mục 6.3 — thêm `exp` như 1 cột thứ 5 vào cùng bảng `tiers[]` trong `data/level-growth.json` (không tạo bảng riêng), nhưng dùng **biến thể không trần** `growthBonusForDepth` vì level tầng không còn cap 100 (mục 6.10):

```
expReward(archetype, floorDepth) = archetype.expReward + growthBonusForDepth("exp", floorDepth)
```

Quái boss nhân thêm hệ số `eliteMultiplier.exp` (đề xuất ban đầu: **x4** — hạ boss là cột mốc "lên tầng", thưởng nhiều hơn hẳn cộng dồn từ quái thường).
...
**⚠️ Số liệu ở 2 bảng trên (expReward mỗi archetype, expCost mỗi tier, hệ số elite x4) là đề xuất ban đầu để có bản chạy được — chưa qua playtest, sẽ cần chỉnh khi có dữ liệu chơi thật, giống mọi bảng số khác trong tài liệu này.**
