...
- **README.md is an introduction, not a mechanics reference.** It should sell the game and get someone playing — not explain how systems work internally (exact targeting rules, formulas, tier thresholds, equip/discard edge cases, etc.). Keep the Features section to one short line per feature; save the "how it actually works" writeup for `docs/developer-guide.md`/`docs/technical-decisions.md`/`docs/gameplay-decisions/*` instead. Class/monster counts and any other README facts still need to stay accurate and in sync with the data (see the rule above) — just don't over-explain them.
...
- **Every stat/parameter that can be rebalanced or changed during maintenance (skills, abilities, classes, monsters, items, artifacts, events, status effects, level-growth curves...) lives in `data/*.json`, never hardcoded in TypeScript.** Engine/loader code (`src/data/*.ts`, `src/engine/*`) must handle any valid combination of values from the JSON without requiring a code change — adding a new skill effect value, a new class stat, or a new monster trait should only ever mean editing JSON.
