...
/**
 * What a status actually does, composed from its own mechanical fields — shown alongside (not
 * instead of) a status's hand-written `description`. Covers every mechanical field a status can
 * carry, so no definition should reach the `item.effectNoPerTurn` fallback; `test/items.test.ts`
 * asserts exactly that, which is what makes adding a new mechanical field without a branch here a
 * test failure rather than a silent "no per-turn effect" on screen.
 */
