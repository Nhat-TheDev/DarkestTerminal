...
/** Cumulative EXP required to reach `level` from level 1 (§6.9) — same cumulative-sum shape as growthBonus, clamped to MAX_LEVEL (character level still caps there, unlike floor depth). */
export function expCostForLevel(level: number): number {
  const clamped = Math.max(1, Math.min(MAX_LEVEL, level));
  if (clamped <= 1) return 0;
  let total = 0;
  for (let l = 2; l <= clamped; l++) {
    total += tierFor(l).expCost;
  }
  return Math.floor(total);
}
