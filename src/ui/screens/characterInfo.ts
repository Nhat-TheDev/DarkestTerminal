...
export function renderMain(game: Game, ui: CharacterInfoUiState): StyledText | string {
  ...
  const boost = artifactStatBoostSum(character, {
    attack: exhausted(baseStats.attack),
    defense: exhausted(baseStats.defense),
    maxHp: baseStats.maxHp,
    maxMp: baseStats.maxMp,
  });
  const abilityBoost = (stat: "attack" | "defense") => abilityWidenedStatBoost(character, stat, exhausted(baseStats[stat]));
  ...
  const lines: TextChunk[][] = [
    ...
    ...statLines(
      ...
      isExhausted,
      0,
      abilityWidenedStatBoost(character, "magicPower", exhausted(baseStats.magicPower)),
      0,
