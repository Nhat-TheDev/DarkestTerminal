...
const ENTITY_TYPES = ["monster", "artifact", "item", "status", "monster-skill"] as const;
...
const ENTITY_FILES: Record<EntityType, string> = {
  ...
  item: "items.json",
  status: "status-effects.json",
  "monster-skill": "monster-skills.json",
};
...
function validateEntry(entry: unknown): string | null {
  ...
  if (typeof id !== "string" || !/^[a-z][a-z0-9-]*$/.test(id)) {
    return "id must start with a lowercase letter and contain only lowercase letters/digits/hyphens.";
  }
  if (typeof name !== "string" || name.length === 0) return "name is required.";
  if (typeof description !== "string" || description.length === 0) return "description is required.";
  return null;
}
...
Bun.serve({
  ...
  async fetch(req) {
    ...
    if (req.method === "GET" && url.pathname === "/api/sprites") {
      const data = await readFile(SPRITES_PATH, "utf8");
    ...
    if (req.method === "POST" && url.pathname === "/api/sprites") {
      ...
      data[category][id] = sprite as SpriteData;
      await writeFile(SPRITES_PATH, JSON.stringify(data, null, 2) + "\n", "utf8");
      return json({ ok: true });
    }
    ...
    if (req.method === "POST" && url.pathname === "/api/edit/class") {
      let body: { classId?: unknown; base?: unknown; growthWeights?: unknown };
      try { body = (await req.json()) as typeof body; } catch { return badRequest("Body is not valid JSON."); }
    ...
    if (req.method === "POST" && url.pathname === "/api/edit/monster") {
    ...
    if (req.method === "POST" && url.pathname === "/api/edit/balance") {
    ...
    if (req.method === "POST" && url.pathname === "/api/edit/level-growth") {
      ...
      for (const [field, key] of [["eliteMultiplier", body.eliteMultiplier], ["bossMultiplier", body.bossMultiplier]] as const) {
    ...
    if (req.method === "GET" && url.pathname === "/api/monster-races") {
      return new Response(await readFile(path.join(DATA_DIR, "monster-races.json"), "utf8"), { headers: { "content-type": "application/json" } });
    }

    if (req.method === "GET" && url.pathname === "/api/monster-traits") {
      return new Response(await readFile(path.join(DATA_DIR, "monster-traits.json"), "utf8"), { headers: { "content-type": "application/json" } });
    }

    const dataMatch = url.pathname.match(/^\/api\/data\/([a-z-]+)(?:\/([a-z0-9-]+))?$/);
    if (dataMatch) {
      const [, type, id] = dataMatch;
      const entityType = type !== undefined && isEntityType(type) ? type : null;
      if (!entityType) return badRequest(`Unknown data type "${type}". Must be one of: ${ENTITY_TYPES.join(", ")}.`);
      const file = ENTITY_FILES[entityType];

      if (req.method === "GET" && !id) {
        return json(await readJsonFile(file));
      }

      if (req.method === "POST" && !id) {
        let body: unknown;
        try { body = await req.json(); } catch { return badRequest("Body is not valid JSON."); }
        const entryError = validateEntry(body);
        if (entryError) return badRequest(entryError);
...
console.log(`Game editor running at http://localhost:${PORT}`);
